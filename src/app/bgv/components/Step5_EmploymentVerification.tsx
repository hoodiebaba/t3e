...
interface Step5Props {
  formData: {
    ...
    [key: string]: any; // For other formData properties
  };
...
const employmentDocumentTypes = ["Offer Letter", "Appointment Letter", "Experience Letter", "Relieving Letter", "Salary Slip (Last Month)", "Bank Statement (Salary Credit)", "Form 16", "Other"];
...
const Step5_EmploymentVerification: React.FC<Step5Props> = ({
  ...
  const employmentEntries = formData.employmentVerification || [
    { id: Date.now(), employerName: '', designation: '', companyAddress: '', joiningMonth: '', joiningYear: '', lastWorkingMonth: '', lastWorkingYear: '', isPresentEmployee: false, reasonForLeaving: '', uploadedDocuments: [] },
  ];
  ...
  const handleEntryChange = (index: number, e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    if (name === "isPresentEmployee") {
      ...
      if (checked) { // If present, clear last working date
        currentEntry.lastWorkingMonth = '';
        currentEntry.lastWorkingYear = '';
        currentEntry.reasonForLeaving = '';
      }
    } else {
      currentEntry[name as keyof Omit<EmploymentEntryData, 'id' | 'uploadedDocuments' | 'isPresentEmployee'>] = value;
    }
  ...
  const addEmploymentEntry = () => {
    const newEntry: EmploymentEntryData = {
      id: Date.now(), employerName: '', designation: '', companyAddress: '', joiningMonth: '', joiningYear: '', lastWorkingMonth: '', lastWorkingYear: '', isPresentEmployee: false, reasonForLeaving: '', uploadedDocuments: [],
    };
  ...
  const addDocumentField = (entryIndex: number) => {
    const updatedEntries = [...employmentEntries];
    const entryDocs = updatedEntries[entryIndex].uploadedDocuments || [];
    const newDoc: DocumentFile = { id: `emp_doc_${Date.now()}`, documentType: '', file: null, fileName: '', fileType: '', previewUrl: null };
  ...
  const handleNextClick = () => {
    ...
    employmentEntries.forEach(entry => {
      if (entry.employerName || entry.designation || entry.joiningYear ) { // If any core field is filled
        if (!entry.employerName || !entry.designation || !entry.joiningYear || (!entry.isPresentEmployee && !entry.lastWorkingYear) ) {
            alert(`Please fill all required fields (Employer, Designation, Joining Year, Last Working Year unless 'Present') for Employment Record: ${entry.employerName || 'New Record'}.`);
            isSectionValid = false;
            return; // Exit forEach early if one entry is invalid
        }
  ...
  return (
    <div className={styles.formStep}>
      ...
      {employmentEntries.map((entry, index) => (
        <div key={entry.id} className={styles.entryBlock}>
          <div className={styles.entryHeader}>
            ...
            {employmentEntries.length > 0 && ( // Show remove only if there are entries
              <button type="button" onClick={() => removeEmploymentEntry(index)} className={styles.removeEntryButton} disabled={isSubmitting}>
                Remove This Record
              </button>
            )}
          </div>
          ...
          <div className={styles.formRow}>
            <div className={styles.formField}>
              <label htmlFor={`employerName_${index}`} className={styles.label}>Employer/Company Name <span className={styles.requiredAsterisk}>*</span></label>
              <input type="text" name="employerName" id={`employerName_${index}`} value={entry.employerName} onChange={(e) => handleEntryChange(index, e)} className={styles.input} disabled={isSubmitting} />
            </div>
            <div className={styles.formField}>
              <label htmlFor={`designation_${index}`} className={styles.label}>Designation <span className={styles.requiredAsterisk}>*</span></label>
              <input type="text" name="designation" id={`designation_${index}`} value={entry.designation} onChange={(e) => handleEntryChange(index, e)} className={styles.input} disabled={isSubmitting} />
            </div>
          </div>

          <div className={styles.formField}>
            <label htmlFor={`companyAddress_${index}`} className={styles.label}>Company Address (Optional)</label>
            <textarea name="companyAddress" id={`companyAddress_${index}`} value={entry.companyAddress} onChange={(e) => handleEntryChange(index, e)} className={styles.textarea} rows={2} disabled={isSubmitting}></textarea>
          </div>

          <div className={styles.formRow}>
            <div className={styles.formField}>
              <label htmlFor={`joiningMonth_${index}`} className={styles.label}>Joining Month <span className={styles.requiredAsterisk}>*</span></label>
            ...
            <div className={styles.formField}>
              <label htmlFor={`joiningYear_${index}`} className={styles.label}>Joining Year <span className={styles.requiredAsterisk}>*</span></label>
          ...
          <div className={styles.formFieldInline}>
            <input type="checkbox" name="isPresentEmployee" id={`isPresentEmployee_${index}`} checked={!!entry.isPresentEmployee} onChange={(e) => handleEntryChange(index, e)} className={styles.checkbox} disabled={isSubmitting} />
            <label htmlFor={`isPresentEmployee_${index}`} className={styles.checkboxLabel}>Currently Working Here</label>
          </div>
          ...
          {!entry.isPresentEmployee && (
            <>
              <div className={styles.formRow}>
                <div className={styles.formField}>
                  <label htmlFor={`lastWorkingMonth_${index}`} className={styles.label}>Last Working Month <span className={styles.requiredAsterisk}>*</span></label>
                  <select name="lastWorkingMonth" id={`lastWorkingMonth_${index}`} value={entry.lastWorkingMonth} onChange={(e) => handleEntryChange(index, e)} className={styles.select} disabled={isSubmitting}>
                    <option value="">Month</option>{months.map(m => <option key={m.value} value={m.value}>{m.label}</option>)}
                  </select>
                </div>
                <div className={styles.formField}>
                  <label htmlFor={`lastWorkingYear_${index}`} className={styles.label}>Last Working Year <span className={styles.requiredAsterisk}>*</span></label>
                  <select name="lastWorkingYear" id={`lastWorkingYear_${index}`} value={entry.lastWorkingYear} onChange={(e) => handleEntryChange(index, e)} className={styles.select} disabled={isSubmitting}>
                    <option value="">Year</option>{years.map(y => <option key={y} value={y}>{y}</option>)}
                  </select>
                </div>
              </div>
              <div className={styles.formField}>
                <label htmlFor={`reasonForLeaving_${index}`} className={styles.label}>Reason For Leaving (Optional)</label>
                <input type="text" name="reasonForLeaving" id={`reasonForLeaving_${index}`} value={entry.reasonForLeaving} onChange={(e) => handleEntryChange(index, e)} className={styles.input} disabled={isSubmitting} />
              </div>
            </>
          )}
            ...
            <div key={doc.id} className={styles.documentUploadRow}>
              <div className={styles.formField}>
                ...
                <select name="docType" id={`docType_${docIndex}`} value={doc.documentType} onChange={(e) => handleDocumentChange(index, docIndex, e)} className={styles.select} disabled={isSubmitting}>
                  ...
                  {employmentDocumentTypes.map(type => <option key={type} value={type}>{type}</option>)}
                </select>
              </div>
              <div className={styles.formField}>
      ...
      <button type="button" onClick={addEmploymentEntry} className={styles.addMoreEntryButton} disabled={isSubmitting}>
