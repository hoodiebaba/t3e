...
<button className={styles.filterModalBtn} onClick={() => setShowFilterModal(false)}>
          ...
          <div className={styles.modalContent}>
            <div style={{ display: 'flex', justifyContent: "space-between", alignItems: "center" }}>
              <h3>
                ...
                {modalStep === 'avf' && "Digital Address Verification Form"}
  ...
  {/*
    Agar window object exist nahi karta (server-side render), to fallback use karo
    production me hamesha window.location.origin hi aayega jab page load ho chuka ho
  */}
  {(() => {
    const baseUrl = typeof window !== 'undefined' ? window.location.origin : 'http://localhost:3000';
    const fullLink = generatedLink ? `${baseUrl}/avf?id=${generatedLink}` : '';
    return (
      <>
...
/>
                ...
                <td>
                 {(() => {
  const baseUrl = typeof window !== 'undefined' ? window.location.origin : 'http://localhost:3000';
  const fullLink = `${baseUrl}/avf?id=${form.token}`;
  return (
    <>
      <a
        href={fullLink}
